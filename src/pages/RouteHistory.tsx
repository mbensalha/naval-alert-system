      ...
      <div className="flex flex-1">
        ...
        <main className="flex-1 p-6 overflow-auto">
          <div className="flex items-center gap-4 mb-8">
            ...
            <h1 className="text-4xl font-bold text-shadow text-slate-900">Historique des Routes</h1>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-6">
              <Card className="bg-white/90 backdrop-blur-sm rounded-lg shadow-lg p-6">
                <CardHeader className="flex flex-row items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" disabled={isTracking} onClick={handleStartTracking} className="bg-green-500 text-white hover:bg-green-600">
                      <Flag className="mr-2 h-4 w-4" />
                      Démarrer
                    </Button>
                    <Button variant="outline" disabled={!isTracking} onClick={handleStopTracking} className="bg-red-500 text-white hover:bg-red-600">
                ...
                <CardContent>
                  {isTracking ? <div className="bg-blue-50 p-4 rounded-md border border-blue-200">
              ...
              <div className="bg-white/90 backdrop-blur-sm rounded-lg shadow-lg p-6">
                ...
                {tracks.length === 0 ? <p className="text-gray-500">Aucune route enregistrée.</p> : <Accordion type="single" collapsible className="space-y-2">
                    {tracks.map(track => <AccordionItem key={track.id} value={track.id} className={cn("border border-gray-200 rounded-md overflow-hidden", selectedTrackId === track.id && "border-2 border-blue-500")}>
                        ...
                        <AccordionContent className="p-4 bg-gray-50">
                          ...
                          <div className="mb-4">
                            ...
                            <p className="flex items-center">{track.points.length} points enregistrés 
                              {track.points.length > 0 && <Navigation className="h-4 w-4 ml-2 text-blue-500" />}
                            </p>
                          </div>
                          <Button size="sm" onClick={() => handleViewTrack(track.id)} className="text-blue-500 hover:text-blue-600">
            ...
            <Card className="bg-navy text-white border-none shadow-lg overflow-hidden flex flex-col h-full">
              ...
              <CardContent className="flex-1 p-0 relative min-h-[500px]">
                <div ref={mapContainerRef} className="w-full h-full">
                  {!selectedTrackId && <div className="absolute inset-0 flex items-center justify-center bg-navy-light/50 z-10">
